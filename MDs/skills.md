...
## Squad-from-pasted-list mode (2026-05-20 — for Amir's Thursday shape)
...
**Reserves-block parsing — three-layer defence** (because Sonnet kept splitting `Reserves:` off as a separate "list" entry, dropping it):
   - **(a) prompt few-shot** with the exact paste-list-with-Reserves format and the required single-entry output.
   - **(b) server-side merge by waMessageId** — if the LLM emits multiple entries for one message, the longest `names` wins as the playing squad and the rest collapse into `reserves`.
   - **(c) deterministic backstop**: if `reserves` is still empty but the source body literally contains `Reserves:`/`Subs:`/`Standby:`, scan the body and extract numbered names after the header. This is **structural-label parsing** (an explicit section marker the human typed), NOT regex-guessing user intent — the user's "don't regex IN/OUT" rule doesn't apply here.

**Regression gate:** `scripts/test-squad-from-list.ts` — 53 checks, live prod, self-wiping. Replays the exact 12 messages from Amir's 17 May chat. Asserts: 11 GroupMessage rows archived, 10 aliases learned (incl. `~T → "tharan"`), 14 CONFIRMED + 1 BENCH attendance rows, 5 guests provisioned with no phone (Ehtisham/Trevell/Faris/Usama/Martin), idempotent 2nd run, Sutton untouched. Run: `npx tsx --env-file=.env scripts/test-squad-from-list.ts`.
