...
vi.mock("@/lib/db", () => ({
  db: {
    ...
    // The Elo the leaderboard ranks on moved from `User.matchRating` to
    // `Membership.matchRating` on 2026-09-19 (club-scoped ratings,
    // slice 5), so the value this file's fixtures supply now arrives
    // through the membership query. Nothing about the inactivity rule
    // depends on where the number comes from — it filters rows and
    // reads no rating at all — but the mock has to answer the query the
    // code actually makes, or every table comes back empty.
...
function seed(s: Seed) {
  ...
  userFindMany.mockResolvedValue(s.users.map((u) => ({ id: u.id, name: u.name })));
  membershipFindMany.mockImplementation(async (args: Record<string, unknown>) => {
    const where = (args.where ?? {}) as { userId?: { in?: string[] } };
    const wanted = where.userId?.in ?? s.users.map((u) => u.id);
    return s.users
      .filter((u) => wanted.includes(u.id))
      .map((u) => ({ userId: u.id, matchRating: u.matchRating }));
  });
