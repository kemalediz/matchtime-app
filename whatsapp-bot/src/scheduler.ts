...
export function initScheduler(waDriver: WaDriver, orgConfigs: Org[]) {
...
async function executeInstruction(instr: DueInstruction, groupId: string): Promise<void> {
  ...
  try {
    if (instr.kind === "group-message") {
      ...
      // @<phone> inline; WhatsApp swaps those for proper tags when the
      // matching id appears in this array. The driver owns the id form,
      // because the suffix is library-specific (`@c.us` under
      // whatsapp-web.js, `@s.whatsapp.net` under Baileys) and a
      // find-and-replace across this file is exactly how that goes wrong.
      type GroupMessageWithMentions = typeof instr & { mentions?: string[] };
      const withMentions = instr as GroupMessageWithMentions;
      const msg = await driver.sendTextWithMentions(
        groupId,
        instr.text,
    ...
    if (instr.kind === "group-poll") {
      const msg = await driver.sendPoll(
        groupId,
        instr.question,
        instr.options,
        instr.multi ?? false,
    ...
    if (instr.kind === "dm") {
      try {
        const msg = await withTimeout(
          driver.sendDirectText(instr.phone, instr.text),
          SEND_TIMEOUT_MS,
    ...
    if (instr.kind === "bench-prompt") {
      // Post in the group (@mention the user).
      const msg = await driver.sendTextWithMentions(groupId, instr.text, [instr.phone]);
      await ackInstruction({
        key: instr.key,
        kind: instr.kind,
        matchId: instr.matchId,
    ...
    if (instr.kind === "update-reaction") {
      ...
      await reactAndReport(driver, instr.waMessageId, instr.emoji, "update-reaction");
      await ackInstruction({
        key: instr.key,
        kind: instr.kind,
      });
